...
const Navbar = () => {
  ...
  const [isTop, setIsTop] = useState(true); // New state to detect whether we're at the top

  useEffect(() => {
    const handleScroll = () => {
      setIsScrolled(window.scrollY > 50); // Change when you scroll down 50px
      setIsTop(window.scrollY === 0); // Detect if we're at the top of the page
    };
  ...
  return (
    <header
      ...
      } ${isTop ? "h-screen" : "h-20"}`} // Full height when at top, reduced height when scrolled
      style={{
          ...
          <picture className="h-full w-40 sm:h-24 sm:w-48 md:h-28 md:w-52 block">
            ...
            <img
              ...
              className={`object-contain w-full h-full transition-all duration-300 ${
                isScrolled ? "h-12" : "h-24" // Change logo size on scroll
              }`}
            />
          </picture>
        </a>
